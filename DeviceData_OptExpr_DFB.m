function dati=DeviceData_OptExpr_DFB(alphaf, kcoupling, eta_i) %#codegen
...
'alphaf',       alphaf,...
'f0',           1.6*10^(-7),...   % 1.1 original value (um^3)
'Vol',          2200,...  %  (um^3)
'Gamma',        0.15,... %(Ghz) 
'epsb',         3.3^2, ...
'kcoupling', kcoupling, ...%(cm^-1);
'eta_i', eta_i ... %intrinsic efficiency
);
end
...
function A=Alfa_i(z)  %3.8 cm^-1: waveguide losses + DFB losses (4)
    A=ones(size(z))*(10+11.8);             
end
